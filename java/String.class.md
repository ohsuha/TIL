...
## 내부 문자열 저장 방식 : Compact Strings
- 자바 9 이전에는 내부적으로 char[] 배열을 사용하여 문자열을 저장했다.
  - 이 배열은 UTF-16 인코딩을 사용하여 각 문자를 16비트로 표현했다.
- 이후에는 byte[] 배열을 사용하여 문자를 저장한다. 이 방식에서는 문자열이 LATIN1 (ISO-8859-1) 또는 UTF-16 인코딩으로 저장되고 실제로는 byte[]배열과 coder라는 필드를 사용해 관리한다.
  - byte[] 는 문자열 데이터를 저장하는 byte[]이고 coder는 인코딩 방식을 나타내는 필드로 LATIN1, UTF-16 값을 가진다.
- 영어 문자열과 같은 경우에는, 문자열이 ASCII 문자로만 구성되어 있습니다. ASCII 문자는 7비트로 표현할 수 있으며, 8비트(1바이트)로 충분히 표현할 수 있습니다.
- 따라서, 이러한 경우에 모든 문자를 16비트(2바이트)로 저장하는 char[]는 불필요하게 메모리를 많이 차지하게 됩니다.
  - LATIN1 (ISO-8859-1) 인코딩: ASCII 문자나 유럽의 많은 언어들을 표현하는 데 사용됩니다. 이 인코딩 방식에서는 각 문자가 1바이트로 저장됩니다.
  - UTF-16 인코딩: ASCII를 넘어서는 문자를 저장해야 할 경우, UTF-16을 사용하여 각 문자를 2바이트로 저장합니다.
- Java 9 이후의 String 클래스는 문자열이 모두 1바이트로 표현 가능한 경우 byte[] 배열을 사용해 메모리를 절약합니다. 예를 들어, "Hello"와 같은 문자열은 char[]로 저장하면 10바이트를 사용하지만, byte[]로 저장하면 5바이트만 사용합니다.
 
### 한글은?
- 한글이 포함된 문자열은 UTF-16 인코딩으로 저장되기 때문에 coder 필드는 UTF-16을 나타내는 값을 가지게 됩니다.
- 한글 문자열의 경우, 기존의 char[] 방식과 byte[] 방식의 메모리 사용량은 거의 동일합니다. 왜냐하면 한글 문자 하나를 표현하는 데는 여전히 2바이트가 필요하기 때문입니다.
- 한글 문자열은 byte[]로 저장되지만, 각 문자가 2바이트를 차지하게 되어 메모리 사용량은 기존의 char[] 방식과 유사합니다.
- Compact Strings의 주요 이점은 ASCII 문자로 이루어진 문자열에서 발생하며, 한글과 같은 비ASCII 문자에서는 메모리 절감 효과가 크게 나타나지 않습니다.
